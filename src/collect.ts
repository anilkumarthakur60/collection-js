class Collection<T> {
  ...
  mapSpread<U>(callback: (...args: T extends (infer I)[] ? I[] : never) => U): Collection<U> {
    return new Collection(this.items.map((item) => callback(...(item as T extends (infer I)[] ? I[] : never))));
  }
  ...
  takeWhile(callback: (item: T, index: number) => boolean): Collection<T> {
    return new Collection(
      this.items.slice(0, this.items.findIndex((item, index) => !callback(item, index)))
    );
  }
  ...
  whereNotNull<K extends keyof T>(key: K): Collection<T> {
    return new Collection(this.items.filter((item) => item[key] !== null && item[key] !== undefined));
  }
