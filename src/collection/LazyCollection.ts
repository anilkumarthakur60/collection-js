...
export class LazyCollection<T> implements Enumerable<T> {
  ...
  // ─── Static factories ───────────────────────────────────────────────────────
  static make<T>(source: LazySource<T> = []): LazyCollection<T> { return new LazyCollection<T>(source) }
  static empty<T>(): LazyCollection<T> { return new LazyCollection<T>([]) }
  ...
  // ─── Conversion ─────────────────────────────────────────────────────────────
  all(): T[] { return [...this.source()] }
  toArray(): T[] { return this.all() }
  toJson(): string { return JSON.stringify(this.all()) }
  toJSON(): T[] { return this.all() }
  toMap<K, V>(keyFn: (item: T, i: number) => K, valueFn: (item: T, i: number) => V): Map<K, V> {
    const out = new Map<K, V>()
    let i = 0
    for (const item of this.source()) { out.set(keyFn(item, i), valueFn(item, i)); i++ }
    return out
  }
  toSet(): Set<T> { return new Set(this.source()) }
  collect(): Collection<T> { return new Collection(this.all()) }
  ...
  search(target: T | Predicate<T>, strict = false): number | false { return ops.searchOf(this.all(), target, strict) }
  has(key: string | readonly string[]): boolean { return ops.hasKey(this.all(), Array.isArray(key) ? (key as readonly string[]) : [key as string]) }

  // ─── Aggregation ────────────────────────────────────────────────────────────
  sum(by?: RetrieverInput<T, number>): number { return ops.sumOf(this.all(), by) }
  average(by?: RetrieverInput<T, number>): number { return ops.averageOf(this.all(), by) }
  avg(by?: RetrieverInput<T, number>): number { return ops.averageOf(this.all(), by) }
  max(by?: RetrieverInput<T, number>): number | undefined { return ops.maxOf(this.all(), by) }
  min(by?: RetrieverInput<T, number>): number | undefined { return ops.minOf(this.all(), by) }
  median(by?: RetrieverInput<T, number>): number | undefined { return ops.medianOf(this.all(), by) }
  mode(by?: RetrieverInput<T>): unknown[] | undefined { return ops.modeOf(this.all(), by) }
  percentage(predicate: Predicate<T>, precision: number = 2): number { return ops.percentageOf(this.all(), predicate, precision) }
  ...
  whereInStrict(key: string, values: readonly unknown[]): LazyCollection<T> { return this.whereIn(key, values, true) }
  ...
  whereNotInStrict(key: string, values: readonly unknown[]): LazyCollection<T> { return this.whereNotIn(key, values, true) }
  ...
  whereNull(key: string): LazyCollection<T> { return this.filter((item) => dataGet(item, key) == null) }
  whereNotNull(key: string): LazyCollection<T> { return this.filter((item) => dataGet(item, key) != null) }
  ...
  flatten(depth: number = Infinity): LazyCollection<unknown> {
    ...
    function* helper(it: Iterable<unknown>, remaining: number): Generator<unknown> {
      for (const v of it) {
        ...
        else if (isPlainObject(v) && remaining > 0) yield* helper(Object.values(v as Record<string, unknown>), remaining - 1)
        else yield v
      }
    }
  ...
  skipWhile(predicate: Predicate<T>): LazyCollection<T> {
    ...
    return new LazyCollection<T>(function* () {
      ...
      let i = 0
      for (const item of src()) {
        if (!started && predicate(item, i)) { i++; continue }
        started = true
        yield item
        i++
      }
  ...
  slice(start: number, length?: number): LazyCollection<T> {
    if (start < 0 || (length !== undefined && length < 0)) return new LazyCollection<T>(ops.sliceOf(this.all(), start, length))
    return this.skip(start).take(length === undefined ? Infinity : length)
  }

  forPage(page: number, perPage: number): LazyCollection<T> { return this.skip((page - 1) * perPage).take(perPage) }
  ...
  chunk(size: number): LazyCollection<LazyCollection<T>> {
    ...
    return new LazyCollection<LazyCollection<T>>(function* () {
      ...
      for (const item of src()) {
        buf.push(item)
        if (buf.length === size) { yield new LazyCollection<T>([...buf]); buf = [] }
      }
  ...
  keyBy(by: RetrieverInput<T, PropertyKey>): Record<PropertyKey, T> { return ops.keyByOf<T, PropertyKey>(this.all(), by) }
  split(groups: number): LazyCollection<LazyCollection<T>> {
    return new LazyCollection<LazyCollection<T>>(ops.splitOf(this.all(), groups).map((c) => new LazyCollection<T>(c)))
  }
  splitIn(groups: number): LazyCollection<LazyCollection<T>> {
    return new LazyCollection<LazyCollection<T>>(ops.splitInOf(this.all(), groups).map((c) => new LazyCollection<T>(c)))
  }

  // ─── Sorting (forces materialisation) ──────────────────────────────────────
  sort(comparator?: Comparator<T>): LazyCollection<T> { return new LazyCollection<T>(ops.sortOf(this.all(), comparator)) }
  sortDesc(): LazyCollection<T> { return new LazyCollection<T>(ops.sortDescOf(this.all())) }
  ...
  sortByDesc(spec: ops.SortBySpec<T>): LazyCollection<T> { return new LazyCollection<T>(ops.sortByDescOf(this.all(), spec)) }
  sortKeys(): LazyCollection<T> { return new LazyCollection<T>(ops.sortKeysOf(this.all() as unknown as readonly object[]) as unknown as T[]) }
  sortKeysDesc(): LazyCollection<T> { return new LazyCollection<T>(ops.sortKeysOf(this.all() as unknown as readonly object[], true) as unknown as T[]) }
  reverse(): LazyCollection<T> { return new LazyCollection<T>(ops.reverseOf(this.all())) }
  shuffle(random?: () => number): LazyCollection<T> { return new LazyCollection<T>(ops.shuffleOf(this.all(), random)) }
  values(): LazyCollection<T> { return new LazyCollection<T>(this.all()) }
  keys(): LazyCollection<string> { return new LazyCollection<string>(ops.keysOf(this.all())) }
  ...
  diffAssoc(other: readonly Partial<T>[] | LazyCollection<Partial<T>>): LazyCollection<T> {
    return new LazyCollection<T>(ops.diffAssocOf(this.all(), this.materialise(other) as readonly Partial<T>[]))
  }
  diffKeys(otherKeys: readonly string[]): LazyCollection<T> {
    return new LazyCollection<T>(ops.diffKeysOf(this.all() as unknown as readonly object[], otherKeys) as unknown as T[])
  }
  ...
  intersectAssoc(other: readonly Partial<T>[] | LazyCollection<Partial<T>>): LazyCollection<T> {
    return new LazyCollection<T>(
      ops.intersectAssocOf(this.all() as unknown as readonly object[], this.materialise(other) as readonly Partial<object>[]) as unknown as T[],
    )
  }
  intersectByKeys(keys: readonly string[]): LazyCollection<T> {
    return new LazyCollection<T>(ops.intersectByKeysOf(this.all() as unknown as readonly object[], keys) as unknown as T[])
  }
  ...
  crossJoin<U>(...others: readonly (readonly U[])[]): LazyCollection<(T | U)[]> {
    return new LazyCollection<(T | U)[]>(
      ops.crossJoinOf<T | U>(this.all() as readonly (T | U)[], ...(others as readonly (readonly (T | U)[])[])),
    )
  }
  unique(by?: RetrieverInput<T>): LazyCollection<T> { return new LazyCollection<T>(ops.uniqueOf(this.all(), by, false)) }
  uniqueStrict(by?: RetrieverInput<T>): LazyCollection<T> { return new LazyCollection<T>(ops.uniqueStrictOf(this.all(), by)) }
  duplicates(by?: RetrieverInput<T>): Record<number, T> { return new Collection(this.all()).duplicates(by) }
  duplicatesStrict(by?: RetrieverInput<T>): Record<number, T> { return new Collection(this.all()).duplicatesStrict(by) }

  only(keys: readonly string[]): LazyCollection<Partial<T>> {
    return new LazyCollection<Partial<T>>(ops.onlyOf(this.all() as unknown as readonly object[], keys) as Partial<T>[])
  }
  except(keys: readonly string[]): LazyCollection<Partial<T>> {
    return new LazyCollection<Partial<T>>(ops.exceptOf(this.all() as unknown as readonly object[], keys) as Partial<T>[])
  }

  flip(): LazyCollection<Record<string, number>> {
    return new LazyCollection<Record<string, number>>([ops.flipOf(this.all() as readonly unknown[])])
  }

  pad(size: number, value: T): LazyCollection<T> { return new LazyCollection<T>(ops.padOf(this.all(), size, value)) }
  multiply(factor: number): LazyCollection<T> { return new LazyCollection<T>(ops.multiplyOf(this.all(), factor)) }
  ...
  tap(callback: (collection: this) => void): this { callback(this); return this }

  // ─── String operations ──────────────────────────────────────────────────────
  implode(glueOrFormatter: string | ((item: T, index: number) => string), keyOrSeparator?: string): string {
    if (typeof glueOrFormatter === 'function') return ops.implodeOf(this.all(), glueOrFormatter, keyOrSeparator ?? '')
    return ops.implodeOf(this.all(), glueOrFormatter, undefined, keyOrSeparator)
  }

  join(glue: string, finalGlue?: string): string { return ops.joinOf(this.all(), glue, finalGlue) }
  ...
  whenEmpty(cb: (c: this) => this | void, fb?: (c: this) => this | void): this { return this.when(this.isEmpty(), cb, fb) }
  whenNotEmpty(cb: (c: this) => this | void, fb?: (c: this) => this | void): this { return this.when(this.isNotEmpty(), cb, fb) }
  unlessEmpty(cb: (c: this) => this | void): this { return this.whenNotEmpty(cb) }
  unlessNotEmpty(cb: (c: this) => this | void): this { return this.whenEmpty(cb) }
  ...
  withHeartbeat(intervalMs: number, callback: () => void): LazyCollection<T> {
    ...
    return new LazyCollection<T>(function* () {
      ...
      for (const item of src()) {
        const now = Date.now()
        if (now - lastTick >= intervalMs) { callback(); lastTick = now }
        yield item
      }
    })
  }
