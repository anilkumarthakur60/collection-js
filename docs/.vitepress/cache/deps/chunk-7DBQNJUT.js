...
var toDisplayString = (val) => {
  return isString2(val)
    ...
    : val == null
      ? ''
      : isArray2(val) ||
          (isObject2(val) && (val.toString === objectToString2 || !isFunction2(val.toString)))
...
If this is a native custom element, make sure to exclude it from component resolution via compilerOptions.isCustomElement.`
    ...
    warn$1(`resolve${capitalize2(type.slice(0, -1))} can only be used in render() or setup().`)
  }
}
function resolve(registry, name) {
  return (
    registry &&
    (registry[name] || registry[camelize2(name)] || registry[capitalize2(camelize2(name))])
  )
}
        ...
        if (true) {
          for (const key in props) {
            ...
            if (
              ...
              (isOn(key) && !isReservedProp2(key)) || // force hydrate v-bind with .prop modifiers
              key[0] === '.'
            ) {
...
function propHasMismatch(el, key, clientValue, vnode, instance) {
  ...
  if (key === 'class') {
    ...
    (el instanceof SVGElement && isKnownSvgAttr2(key)) ||
    (el instanceof HTMLElement && (isBooleanAttr2(key) || isKnownHtmlAttr2(key)))
  ) {
...
function watchPostEffect(effect2, options) {
  return doWatch(effect2, null, true ? extend2({}, options, { flush: 'post' }) : { flush: 'post' })
}
function watchSyncEffect(effect2, options) {
  return doWatch(effect2, null, true ? extend2({}, options, { flush: 'sync' }) : { flush: 'sync' })
}
...
function doWatch(source, cb, { immediate, deep, flush, once, onTrack, onTrigger } = EMPTY_OBJ2) {
  if (cb && once) {
    const _cb = cb
    cb = (...args) => {
      _cb(...args)
...
function patchAttr(el, key, value, isSVG, instance, isBoolean2 = isSpecialBooleanAttr3(key)) {
  if (isSVG && key.startsWith('xlink:')) {
    ...
    if (value == null || (isBoolean2 && !includeBooleanAttr2(value))) {
      ...
      el.setAttribute(key, isBoolean2 ? '' : isSymbol3(value) ? String(value) : value)
    }
  }
}
      ...
      if (props && !isArray3(props)) {
        for (const key in props) {
          ...
          if (opt === Number || (opt && opt.type === Number)) {
            ...
            ;(numberProps || (numberProps = /* @__PURE__ */ Object.create(null)))[camelize3(key)] =
              true
          }
        }
      }
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-7DBQNJUT.js.map
